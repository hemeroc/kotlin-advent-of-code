...
fun main() {
    ...
    fun solve1(map: List<CharArray>, antennas: MutableMap<Char, MutableList<Point>>): Int {
    ...
    fun solve2(map: List<CharArray>, antennas: MutableMap<Char, MutableList<Point>>): Int {
        val antiNodes = mutableSetOf<Point>()
        for ((char, points) in antennas.entries) {
            if (char == '.') continue
            for (i in points.indices) {
